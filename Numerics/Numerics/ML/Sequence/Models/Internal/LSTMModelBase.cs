...
public abstract class LSTMModelBase : ISequenceModel, IHasHyperparameters
{
    ...
    protected void CopySharedParametersTo(LSTMModelBase clone)
    {
    ...
    private SequentialModel BuildNetwork(int outputSize)
    {
        ...
        // LSTM layer: returnSequences=false → single hidden-state output
        layers.Add(new LSTMLayer(Features, HiddenSize, returnSequences: false, clipNorm: ClipNorm, seed: seed++));

        // Optional hidden dense layer
        int currentInput = HiddenSize;
        if (HiddenUnits > 0)
        {
